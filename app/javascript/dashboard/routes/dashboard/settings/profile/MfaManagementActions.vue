...
const downloadBackupCodes = () => {
  const installationName =
    window.chatwootConfig?.installationName || 'RomiCars';
  const codesText = `${installationName} Two-Factor Authentication Backup Codes\n\n${props.backupCodes.join('\n')}\n\nKeep these codes in a safe place.`;
  const blob = new Blob([codesText], { type: 'text/plain' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `${installationName.toLowerCase().replace(/\s+/g, '-')}-backup-codes.txt`;
  a.click();
