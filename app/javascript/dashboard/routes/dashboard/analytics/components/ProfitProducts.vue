...
function productAmount(p) {
  ...
  return Number(amt).toLocaleString('es-VE', {
    style: 'currency',
    currency: 'USD',
...
<template>
  <div class="bg-white dark:bg-n-solid-2 rounded-xl border border-n-weak p-5">
    <div class="flex items-center gap-2 mb-4">
      <span class="i-lucide-package size-4 text-n-accent" />
    ...
    <div v-else-if="!available" class="text-center py-8">
      <span
        class="i-lucide-plug-zap size-8 text-n-slate-9 mx-auto block mb-2"
      />
      <p class="text-sm text-n-slate-11 font-medium">
        Profit API no configurada
      </p>
      <p class="text-xs text-n-slate-9 mt-1">
        Agrega PROFIT_API_URL, PROFIT_API_USER y PROFIT_API_PASSWORD al entorno
        del servidor.
      </p>
    </div>
    ...
    <div v-else>
      <div
        v-for="(product, idx) in activeTab === 'top'
          ? productsTop
          : productsBottom"
        :key="idx"
      ...
      >
        <span
          ...
          :class="
            activeTab === 'top' && idx < 3 ? 'text-amber-500' : 'text-n-slate-9'
          "
        >
          {{ idx + 1 }}
        </span>
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-n-slate-9">
            {{ productQty(product) }} unidades
          </p>
        </div>
        <p
          class="text-sm font-semibold text-n-slate-12 tabular-nums flex-shrink-0"
        >
      ...
      <p
        v-if="!(activeTab === 'top' ? productsTop : productsBottom).length"
        class="text-xs text-n-slate-9 text-center py-6"
      >
